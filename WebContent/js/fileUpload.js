...
var uploadTools = {
    ...
    "initWithCleanFile":function(opt){
        ...
        if(!opt.isHiddenCleanBt){
            ...
            $("#"+uploadId+" .uploadBts .cleanFileBt i").css("color","#0087F7");
        }
    },
    /**
     * 初始化选择文件按钮
    ...
    "getShowFileType":function(isImg,fileType,fileName,isImgUrl,fileCodeId){
        ...
        modelStr+="<i class='iconfont icon-trash'></i>";
        modelStr+="</div>";
    ...
    "initWithLayout":function(opt){
        ...
        btsStr += "<div>";
        btsStr += "<div class='selectFileBt'>Choose File  ";
        btsStr += "<i class='iconfont icon-picture'></i>"
        btsStr += "</div></div>";
        //上传按钮
        if(!opt.isHiddenUploadBt){
            btsStr += "<div class='uploadFileBt'>";
            btsStr += "<i class='iconfont icon-cloud-upload'></i>";
            btsStr += " </div>";
        }
        //清理按钮
        if(!opt.isHiddenCleanBt){
            btsStr += "<div class='cleanFileBt'>";
            btsStr += "<i class='iconfont icon-refresh'></i>";
            btsStr += " </div>";
        }
        btsStr += "</div>";
    ...
    "addFileList":function(fileList,opt){
        ...
        if(fileNumber+fileList.length>opt.maxFileNumber){
            alert("Maxmum is "+opt.maxFileNumber);
            return;
        }
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            if(uploadTools.fileIsExit(fileList[i],opt)){
                alert("File（"+fileList[i].name+"）already exists！");
                continue;
            }
            var fileTypeStr =  uploadTools.getSuffixNameByFileName(fileList[i].name);
            //文件大小显示判断
            if(fileSizeLimit!=-1&&fileList[i].size>(fileSizeLimit*1000)){
                alert("File（"+fileList[i].name+"）is out of size！Please use files within "+fileSizeLimit+"KB");
                continue;
            }
            //文件类型判断
            if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeStr,fileTypeArray)){
                ...
                alert("Format not allowed:"+fileList[i].name);
            }
        }
    ...
    "uploadError":function(opt){
        ...
        $("#"+uploadId+" .box .fileItem .status>i").addClass("iconfont icon-remove-circle");
    ...
    "uploadSuccess":function(opt){
        ...
        $("#"+uploadId+" .box .fileItem .status>i").addClass("iconfont icon-ok-circle");
        var progressBar = $("#"+uploadId+" .subberProgress .progress>div");
        progressBar.css("width","0%");
        progressBar.html("0%");
    },
